...
_DTC_IN_TEXT_RE = re.compile(r'\b([A-Z][0-9]{4}|[0-9A-F]{4,6})\b', re.IGNORECASE)
_INSPECTION_WORDS = [
...
class ROValidator:
    ...
    def validate(self, ro_draft: RODraft) -> ValidationReport:
        ...
        if not complaint.strip().upper().startswith("CUST STATES"):
            field_flags.append(FieldFlag(
                ...
                rejection_reason="Complaint must open with CUST STATES per BMW documentation standard",
                severity="hard",
            ))
        ...
        if len(complaint.split()) < 10:
            field_flags.append(FieldFlag(
                ...
                rejection_reason="Complaint too brief — insufficient customer description for claim support",
                severity="hard",
            ))
        ...
        for word in _DIAG_LANGUAGE:
            if word in complaint.lower():
                field_flags.append(FieldFlag(
                    ...
                    rejection_reason="Complaint field contains diagnosis language — move technical findings to Cause field",
                    severity="hard",
                ))
                break
        ...
        if not _DCI_RE.search(cause):
            field_flags.append(FieldFlag(
                ...
                rejection_reason="Cause must reference a BMW Diagnosis Category Identifier per SIB 01 01 20",
                severity="hard",
            ))
        ...
        if not has_dtc_ref and not has_inspection:
            field_flags.append(FieldFlag(
                field="cause",
                flag_description="Cause references neither a DTC nor a visual/mechanical inspection finding",
                rejection_reason="Cause must reference either a DTC or an inspection finding",
                severity="hard",
            ))
        ...
        if not _RCI_RE.search(correction):
            field_flags.append(FieldFlag(
                ...
                rejection_reason="Correction must reference a BMW Repair Category Identifier per SIB 01 01 20",
                severity="hard",
            ))
        ...
        if not ro_draft.labor_op_code or ro_draft.labor_op_code in ("N/A", ""):
            field_flags.append(FieldFlag(
                ...
                rejection_reason="Labor operation code missing — claim cannot be processed without a valid BMW labor op code",
                severity="hard",
            ))
        ...
        if not ro_draft.warranty_compliant:
            field_flags.append(FieldFlag(
                field="correction",
                flag_description="CorrectionWriter flagged one or more warranty rule violations",
                rejection_reason="One or more warranty rules violated — review flagged rules before submission",
                severity="soft",
            ))
    ...
    def format_report(self, validation_report: ValidationReport) -> None:
        ...
        if validation_report.passed:
            print("  [OK] RO PASSED ALL CHECKS -- READY FOR SUBMISSION")
            print(dline)
            return

        print(f"VERDICT: {validation_report.submission_verdict}")
        print(f"Hard issues: {validation_report.hard_count}   Soft issues: {validation_report.soft_count}")
        ...
        for field_name in fields_order:
            ...
            print(f"\n{field_name.upper()}:")
            for flag in flags:
                icon = "[X]" if flag.severity == "hard" else "[!]"
        ...
        if validation_report.rule_violations:
            ...
            for v in validation_report.rule_violations:
                icon = "[X]" if v.severity == "hard" else "[!]"
